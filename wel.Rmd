...
```{r all-data}
reserve_all_fxn("WEL") + 
  ggpubr::stat_regline_equation(label.y = 8) +
...
interact_reserve_interf_fxn("WEL", param = fdom)
```

## Turbidity

```{r turb-interf, dpi = 120}
interact_reserve_interf_fxn("WEL", param = turbidity)
```

## Temperature

```{r temp-interf, dpi = 120}
interact_reserve_interf_fxn("WEL", param = temperature)
```
