...
all_isco_figure <- isco %>% 
  ...
  ggpubr::stat_regline_equation(label.y = 75, label.x = 11) +
  ggpubr::stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`, `~")), 
                   label.y = 71, label.x = 11) + # add R2 and p value
...
isco_interf <- function(param, interact) {
  if (interact == TRUE) {
    m = list(
      l = 80,
      r = 150,
    ...
    if (param > 1) {
      ...
      interact_isco_fDOM %>% layout(margin = m)
    }
    else if (param == 1) {
      ...
      interact_isco_temp %>% layout(margin = m)
    }
    else {
      ...
      interact_isco_turb %>% layout(margin = m)
    }
