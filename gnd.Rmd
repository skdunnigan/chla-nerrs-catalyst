...
```{r all-data}
reserve_all_fxn("GND") + 
  ggpubr::stat_regline_equation(label.y = 22) +
...
```{r isco, dpi = 120}
interact_reserve_isco_fxn("GND")
```

## Tank Samples

```{r tank, dpi = 120}
interact_reserve_tank_fxn("GND")
```
...
interact_reserve_interf_fxn("GND", param = fdom)
```

## Turbidity

```{r turb-interf, dpi = 120}
interact_reserve_interf_fxn("GND", param = turbidity)
```

## Temperature

```{r temp-interf, dpi = 120}
interact_reserve_interf_fxn("GND", param = temperature)
```
