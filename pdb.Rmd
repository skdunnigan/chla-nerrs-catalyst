...
```{r all-data}
reserve_all_fxn("PDB") + 
  ggpubr::stat_regline_equation(label.y = 15) +
...
```{r isco, dpi = 120}
interact_reserve_isco_fxn("PDB")
```

## Tank Samples

```{r tank, dpi = 120}
interact_reserve_tank_fxn("PDB")
```
...
interact_reserve_interf_fxn("PDB", param = fdom)
```

## Turbidity

```{r turb-interf, dpi = 120}
interact_reserve_interf_fxn("PDB", param = turbidity)
```

## Temperature

```{r temp-interf, dpi = 120}
interact_reserve_interf_fxn("PDB", param = temperature)
```
